#	Version of the single pulse research of S. Ransom with a non-biased
#	std estimator taking account of the downsampling.
...
#		Comments :
#	Non-biased std estimator : 1 dof.
#	Std calculated on all the samples.
#	Std estimatation corrected by the new threshold.
#	Decrease the 0.5 extension to 0.1 pc.cm-3 in the plots.
#	Single pulses research in multi processing.
#	Minimum threshold used for the research.
#	Correct the threshold relative for a different downsampling.
...
def main( args ):
	...
	if not opts.threshold :
		opts.threshold = ( Num.sqrt( Num.log( opts.nSamp * opts.nDM ) ) - Num.sqrt( Num.pi / 4 ) ) / Num.sqrt( 1 - Num.pi / 4 )
	...
	if not opts.threshold :
        ...
        for filenm in args:
            ...
            obstime = N * dt
	    if info.N != opts.nSamp :
		SNRfactor = ( Num.sqrt( Num.log( N * opts.nDM ) ) - Num.sqrt( Num.pi / 4 ) ) / ( Num.sqrt( Num.log( opts.nSamp * opts.nDM ) ) - Num.sqrt( Num.pi / 4 ) )
...
#                stds[ii] = Num.sqrt((tmpchunk[detrendlen/40:-detrendlen/40]**2.0).sum() / (0.95*detrendlen))
		stds[ii] = Num.std( tmpchunk , ddof=1 )
	    if info.N != opts.nSamp :
		stds *= SNRfactor
#            stds *= 1.148
            ...
            print "numblocks", numblocks , sort_stds[numblocks/2+1:], sort_stds[numblocks/2:-1]
    ...
    if (opts.makeplot):
        ...
        ppgplot.pgswin(min(DMs)-0.1, max(DMs)+0.1, 0.0, 1.1*max(num_v_DM+[1]))
        ppgplot.pgsch(0.8)
        ...
        ppgplot.pgswin(min(DMs)-0.1, max(DMs)+0.1, opts.threshold, maxsnr)
        ppgplot.pgsch(0.8)
        ppgplot.pgbox("BCNST", 0, 0, "BCNST", 0, 0)
        ppgplot.pgmtxt('B', 2.5, 0.5, 0.5, "DM (pc cm\u-3\d)")
        ...
        ppgplot.pgswin(opts.T_start, opts.T_end, min(DMs)-0.1, max(DMs)+0.1)
        ppgplot.pgsch(0.8)
        ppgplot.pgbox("BCNST", 0, 0, "BCNST", 0, 0)
...
if __name__ == '__main__':
	...
	parser.add_option("-o", "--nbsamp", type="int", dest="nSamp", default=1000000,
                      help="Original number of samples.")
	parser.add_option("-n", "--nbdm", type="int", dest="nDM", default=500,
                      help="Number of tried DMs.")
	parser.add_option("-c", "--ncpus", type="int", dest="ncpus", default=8,
		      help="Number of CPUs to use (default = 8).")
	(opts, args_data) = parser.parse_args()
	...
	if len(args_data)==0:
		...
		for globexp in opts.globexp.split():
			args_data += glob.glob(globexp)

	if opts.makeplot :
		nCPUs = 1
	else :
		if mp.cpu_count() < opts.ncpus :
			nCPUs = mp.cpu_count()
		else :
			nCPUs = opts.ncpus
		part = len( args_data ) // nCPUs + 1
	...
	if (0):
		...
		if opts.makeplot :
			...
			res = []
			ind = 0
			while ( ind * part ) < len( args_data ) :
				work_args = args_data[ ind * part : ( ind + 1 ) * part ]
				ind += 1
				res.append( pool.apply_async( main , ( work_args , ) ) )
			for r in res :
				print r.get()
