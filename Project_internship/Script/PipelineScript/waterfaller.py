...
def get_mask(rfimask, startsamp, N):
    ...
    for blocknum in np.unique(blocknums):
        ...
        chans_to_mask = np.append(chans_to_mask,[171])
        #chans_to_mask = np.arange(100,190)
        print 'chans_to_mask', chans_to_mask
        print 'blockmask shape', blockmask.shape

        #for i in chans_to_mask:
        #    print 'chan to mask', i
            #if chans_to_mask:
        blockmask[:,chans_to_mask] = True
        #print 'blockmask', blockmask
        mask[blocknums==blocknum] = blockmask
    #print 'this is the mask', mask.T
    return mask.T
...
def maskfile(maskfn, data, start_bin, nbinsextra):
    ...
    #print 'masked chans', masked_chans, len(masked_chans)
    # Mask data
    #print 'data before mask', data
    data = data.masked(mask, maskval='median-mid80')
    #data = data.masked(mask, maskval=0)
    #print 'data after mask', data
...
def waterfall(rawdatafile, start, duration, dm=None, nbins=None, nsub=None,\
    ...
    print 'nbins', nbins, 'nbinsextra', nbinsextra, 'start_bin', start_bin
    print 'nchan', rawdatafile.nchan, 'nsamp_per_subint', rawdatafile.nsamp_per_subint
    ...
    print 'data_masked shape', data_masked.shape, np.ma.masked.shape, data.data.shape
    ...
    # Dedisperse
    if dm:
        data.dedisperse(dm, padval='median')
