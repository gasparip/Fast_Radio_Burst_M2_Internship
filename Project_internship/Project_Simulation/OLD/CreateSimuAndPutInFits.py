...
parser.add_argument('-T', dest='PulseDuration', type=float, default=0.100, help='Change the Duration of the pulse')
...
ListTdm=[DM*4.15e3*(1/float((frequencyListCent[-1]**2)))]
if Polynome : AmpliBandFreq = FrenquencyBand(frequencyStart,frequencyEnd,ChanelFreq)
for i in range(0,int(ChanelFreq-1)):
	# Bad List frequencyList += [frequencyList[-1]+round(bw/float((ChanelFreq-1)),7)]
...
for i in range(len(ListTdm)):
	if Polynome : 
		...
		g1 = models.Gaussian1D(energyPulse, ListTdm[i], PulseDuration)
	signalFi = g1(time)
	
	if ScatteringMode :
		Tau=0.00105*(600/float(frequencyListCent[i]))**(4)
